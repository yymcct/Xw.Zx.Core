...
(function (global, factory) {
                    ...
                    try {
                        ...
                        if (url) {
                            if (type == 'png' || type == 'jpg' || type == 'jpeg' || type == 'gif') {
                                ...
                                this$1.$parent.execCommand(Command.CREATE_LINK, url, file.name);
                            }
                        }
                    } catch (err) {
                        this$1.setUploadError(err.toString());
    ...
    var image = {
        ...
        config: {
            ...
            // height: 1600,
            // quality: 80,
            sizeLimit: 512 * 1024 * 1024 * 1024,// 512k
            // upload: {
            //     url: null,
            //     headers: {},
            //     params: {},
    ...
    var template$5 = "<form @submit.prevent=\"createLink\"> <div v-show=\"upload.status=='ready'\"><input type=\"text\" :placeholder=\"$parent.locale['please enter a url']\" v-model=\"url\" maxlength=\"1024\" style=\"margin-right:5px\"><button type=\"submit\">{{$parent.locale[\"create link\"]}}</button> <input type=\"file\" ref=\"file\" style=\"display: none !important\" @change=\"process\" > <button type=\"button\" @click=\"pick\">{{$parent.locale.upload}}</button></div><div v-if=\"upload.status=='progress'\"> {{$parent.locale.progress}}:{{upload.progressComputable ? $parent.locale.unknown : upload.complete}} </div> <div v-if=\"upload.status=='success'\"> {{$parent.locale[\"please wait\"]}}... </div> <div v-if=\"upload.status=='error'\"> {{$parent.locale.error}}:{{upload.errorMsg}} <button type=\"button\" @click=\"reset\">{{$parent.locale.reset}}</button> </div> <div v-if=\"upload.status=='abort'\"> {{$parent.locale.upload}}&nbsp;{{$parent.locale.abort}}, <button type=\"button\" @click=\"reset\">{{$parent.locale.reset}}</button> </div></form>";
    var dashboard$5 = {
        template: template$5,
        data: function data() {
            return {
        ...
        methods: {
            ...
            uploadToServer: function uploadToServer(file) {
                ...
                xhr.onload = function () {
                    ...
                    try {
                        var url = '/dghy/' + json.Addr;
                        var type = json.Name.split('.')[1];
                        if (url) {
                            if (type == 'png' || type == 'jpg' || type == 'jpeg' || type == 'gif') {
                                this$1.$parent.execCommand(Command.INSERT_IMAGE, url);
                            } else {
                ...
                xhr.open('POST', '/jz/XBM_Service.bsp?File', true);
                // if (typeof config.upload.headers === 'object') {
                // Object.keys(config.upload.headers).forEach(function (k) {
     ...
     * @param arg
     */
    RangeHandler.prototype.execCommand = function execCommand(command, arg, name) {
        var this$1 = this;
        switch (command) {
            ...
            case Command.CREATE_LINK: {
                    var textNodes$1 = this;
                    // var fragment = document.createDocumentFragment();
                    var a = document.createElement('a');
                    a.innerHTML = name||arg;
    ...
    var editor = {
        ...
        methods: {
            ...
            execCommand: function execCommand(command, arg, name) {
                this.restoreSelection();
                if (this.range) {
                    new RangeHandler(this.range).execCommand(command, arg, name);
                }
