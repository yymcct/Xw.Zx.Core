<template>
	<el-form :model="parentForm" ref="parentForm" label-width="50px" class="parentForm" v-if="parentType">
		<el-form-item label="名称" prop="nodename" :rules="{ required: true, message: '请输入菜单名称', trigger: 'blur' }">
	...
	<el-form :model="dynamicValidateForm" ref="ruleForm" label-width="100px" class="TreeForm" v-else>
		...
		<el-form-item v-for="(domain, index) in dynamicValidateForm.DATA" :label="'子节点' + (index+1)" :key="domain.key" :prop="'DATA.' + index + '.nodename'"
		 :rules="{
