...
namespace Xw.Zx.Core.Service
{
    public class GuangZhouYinHangParseService : IGuangZhouYinHangParseService
    {
        ...
        private readonly ILogger<GuangZhouYinHangParseService> _logger;
        private readonly XwZxContext _xwZxContext;
        public GuangZhouYinHangParseService(ILogger<GuangZhouYinHangParseService> logger
        ...
        private List<MailSrc> GetNeedParse()
        {
            var mails = _xwZxContext.MailSrcs
               .Where(m => m.MemberId == _memberId
                   && m.From == BankMailUrl.GUANGZHOUYINHANG
                   && m.IsPrased == false
                   && m.Sublic.Contains("广州银行")
